    ...
    <>
      <div className="absolute top-0 left-0 w-full z-10 ">
        <div className="container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent">
          ...
          <img onClick={()=> setShowMobileMenu(true)} src={menuIcon} alt="menu-icon" className="md:hidden  w-7" />
        </div>
        {/* ----------------mobile menu */}
        <div className={`md:hidden ${showMobileMenu ? 'fixed w-full' : 'h-0 w-0'} fixed w-full right-0 top-0 bottom-0 overflow-hidden
         bg-white transition-all` }>
          <div className=" flex justify-end p-6 cursor-pointer">
            <img onClick={()=> setShowMobileMenu(false)} src={crossIcon} className="w-6" alt="cross-icon" />
          </div>
